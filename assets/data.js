...
// https://pixabay.com/fr/illustrations/logo-html-html5-ic%C3%B4ne-2582748/
// https://freebiesupply.com/logos/java-logo-3/

let projects = [
    new project("Chess", "assets/chess.png", "Chess is an abstract strategy game where both sides attempt to checkmate the opponent.\n\nThis recreation of chess was made in Python using the library pygame and stockfish. The squares are highlighted for the last pieces moved as well as the legal moves when clicking on a piece. It also has time controls, which you can modify in the config file. Stockfish is also usable with this project.\n\nRead the README.md file for information on how to use.", ["Python", "Pygame"], "assets/chess.zip", "download"),
    new project("Portfolio", "assets/html.png", "The portfolio website that you are currently looking at. Built on HTML, CSS and JavaScript (with no libraries), it is the place to find out more about me. It contains projects that I have made, tutorials for ICS4U as well as some things about myself. \n\nThis portfolio was made for ICS4U.", ["HTML", "CSS", "JavaScript"], "", "view"),
    new project("Asteroid Alert", "assets/AsteroidAlert.png", "Asteroid alert is a game that follows an astronaut that is scrambling to fix their escape pod in order to save the world from an asteroid. To fix their escape pod, they must go through multiple rooms within the space station to find the necessary parts. Fix the escape pod and save the world. Be unable to fix it and watch the world be destroyed.\n\nMade for ICS2OP culminating with Colin.", ["Processing Java"], "assets/ColinCaiAlexYan_FinalISP.zip", "download"),
    new project("Java Syntax Highlighter", "assets/java.png", "This is a very basic Java syntax highlighter made in JavaScript, designed to be used for code blocks on websites. The syntax.css file allows you to modify the palette or theme to the programmer's own liking.\n\nEvery code block on this website utilizes this syntax highlighter to do the highlighting.", ["JavaScript"], "assets/syntax.zip", "download")
]
...
window.OOP_Basic = [
    ...
    new item("body", "In Java, objects are created from classes, which are like blueprints for the object. Within the classes, there are then attributes and methods. These attributes hold the state of an object. Going back to the table as an object example, the attributes inside of the class could contain the information about the color or the materials. Methods represent an object's behaviors, and can be used to modify the attributes or perform some other action. Finally, in Java, an object is an instance of the class, with its own data values. For example, an object could be a table class with the color set as red and the material be plastic."),
    new item("body", "To create an object in Java, we will invoke the constructor of a class. The constructor of a class is what creates the object. When writing a constructor, it has no return type and the name is the name of the class. Java allows overloading constructors, meaning we can construct constructors with different arguments. This allows us to set up the basics for the class, setting values for attributes and more. If no constructor is present, Java will assign a zero parameter constructor for the class. "),
    ...
    new item("body", "Methods can also be categorized. Two common categories are set and get methods. Set methods take in arguments and change the value of one or more attributes. On the other hand, get methods are methods which will return the value of specific attributes. Both of these methods are designed for other parts of the program to interact with data inside of the object. "),
    new item("code", `// Same Table class as before
    public static void main(String[] args) {
    ...
    new item("body", "One project that utilizes the concept of OOP was the creating software assignment, where we were given 4 classes to comment. One of them was a student class, which was then tested out in a tester class. "),
    new item("code", `public class Student{
        // Usage of attributes
        public String name;
        public int mark1;
        public int mark2;
...
window.OOP_Advanced = [
    ...
    new item("body", "Classes can have additional functionality through inheritance and polymorphism. These are the key features that object oriented programming has. Inheritance allows classes inherit from different classes while polymorphism allows for classes to have many different \"forms\"."),
    ...
    new item("body", "Polymorphism has two neat features, known as method overloading and method overriding. Method overloading is when a method has the same name but different parameters. On the other hand, method overriding is when a method in the parent class is overridden by another method of the same name and parameter later down in the hierarchy."),
    new item("subtitle", "Method Overloading"),
    new item("body", "When method overloading, it requires the method to have the same name but different parameters. The different parameters could be a different number of parameters or parameters of a different type. This allows the Java compiler to figure out which method is being called, as the parameters would be different. This allows for methods that do similar tasks to have the same name, for ease for the programmer."),
    ...
    new item("body", "Method overriding is done when a child class (or any class in the hierarchy) has a method of the same name and parameters as the one in the parent. However, the body of the child class' method might be different than the parent, to suite the needs of the child."),
    ...
    new item("body", "Class casting is where the true power of method overriding comes in. However, unlike primitive data types, the object doesn't change types. If we have a Dog object and we cast it into an Animal class, the Dog object doesn't suddenly turn itself into an Animal object. Rather, the Dog object now gets treated as if it were an Animal object and loses access to all of the methods and attributes specific to the Dog class. Upcasting is when a child gets casted to a parent while downcasting is when a parent gets casted into a child. When upcasting, Java will automatically do it for the programmer. However, downcasting requires the programmer to manually state which child, as a Cat object, cannot be cast into a sibling class, such as a Dog. When a child is upcasted into a parent, it loses access to the child class' specific attributes and variables. However, if a method in the child is overriding a parent method whilst being upcasted, when the method is called, it calls the child's rather than the parent."),
    new item("code", `public class Parent {
        public void task() {
    ...
    new item("body", "Sometimes, programmers want classes that have some or all of its methods to be abstracted. An abstracted method is one that is specially designed to be overridden and in the code, only provides the name, return type and parameters. It is created with the keyword abstract and has no body code. An abstract class is one where one or more of its methods is abstracted. However, it can also contain non-abstract methods. On the other hand, interfaces only contain abstract methods. They cannot contain any other non-abstract methods and only have final variables. Interfaces are created with the keyword interface while abstract classes are created with the keyword, abstract class. The interfaces implemented while abstracted classes are extended."),
    ...
    new item("code", `public class Main extends AbstractTest implements InterfaceTest {
        ...
        public static void main(String[] args) {
            Main m = new Main();
            m.task(); // Abstract's abstract method being overridden
            m.nonAbstract(); // A non-abstract method from the abstract class
            m.otherTask(); // Interface's abstract method being overridden
        }
    }
    `, "Main.java"),
    new item("subtitle", "Common Mistakes"),
    new item("body", "Some common mistakes include whilst upcasting is done automatically by Java, downcasting is not. Therefore, whenever you need to cast it to a child, you must specify which child class it should be casted into. This is to prevent a Dog object from accidentally being cast into its sibling, a Cat class. Another common mistake is that just because a method has a different return type doesn't mean it has been method overloaded. Two methods with different return types are treated to be the same as Java cannot predict which one the programmer wants."),
    new item("subtitle", "Project Demo"),
    new item("body", "One project that made use of inheritance was the fantastical invention assignment. In this assignment, we had to come up with two inventions. The first needed to be a more general or vague invention and then the second would be one that was more specific than the first. This meant that during coding, the first invention would be the parent and the second would be the child, as the second world inherent some features from the first as they are closely related."),
    new item("body", "Part of the parent class"),
    new item("code", `public class OrganQuickSave {
        ...
        // Default constructor initializing the class to that of a brain
    ...
    new item("code", `public class BrainQuickSave extends OrganQuickSave { // Inherits from OrganQuickSame
...
window.Arrays = [
    ...
    new item("code", `import java.util.*;
...
window.Arraylists = [
    ...
    new item("body", "Before we can even use it, we must first import a package. For this, we will use import java.util.ArrayList. To create a new ArrayList, we use the syntax ArrayList<dataType> variableName = new ArrayList<dataType>(). Note that primitive data types must use their wrapper class (e.g int and Integer). Once we have created an ArrayList, we can now add things to their ArrayList. To do that, we can use the syntax .add(element). We can also specify which location to add it to with the overloaded method using the syntax of .add(index, element). This will shift all the other elements after the index to the right. To obtain the element at a specified location, we use the .get(index) syntax. In order to change an element at a specific index, we use the .set(index, element)."),
    new item("body", "Much like arrays, ArrayLists are also 0 based, meaning the first element in the ArrayList is at index 0. Similarly, ArrayLists are also iterable over, meaning you can use a for loop to throw the elements. You can use both a normal for loop utilizing the .size() method to get the length of the array or using the for each loop mentioned in the arrays lesson. On top of that, ArrayLists can also be nested, creating nested ArrayLists."),
...
window.Sorting = [
    ...
    `, "MergeSort.java"),
    new item("subtitle", "Common Mistakes"),
    new item("body", "A common mistake that could be made, especially while implementing merge sort, is mixing up the indexes while performing the merge. Remember that the number of elements between two indexes (inclusive) is the smaller index subtracted from the bigger index plus one. If the plus one is forgotten, then the array becomes too small and errors will arise. "),
    new item("subtitle", "Project Demo"),
    new item("body", "One assignment that utilized a sorting algorithm was the sorting algorithm, where we had to sort a bunch of countries by their name or by their population. After we sorted it, we would then output it to a file. In this assignment, we utilized insertion sort (a sorting algorithm not covered above)."),
...
window.Recursion = [
    ...
    new item("body", "A very common mistake when using recursion is writing a poor base case that doesn't apply to all intended cases. This will cause a stack overflow to happen as some data may never enter the base case. Another common mistake for recursion is using it for the wrong purpose. For example, using recursion on very large data may not be the smartest idea as it may recursively call itself so many times that it throws a stack overflow anyways. Thus, the programmer must choose very carefully when doing recursion."),
    new item("subtitle", "Project Demo"),
    new item("body", "One project that made use of recursion was the recursion assignment. In this assignment, we were given a bunch of tasks to complete utilizing recursion. These tasks included finding the sum of all the numbers from 1 to n as well as the value of an exponent. The first could be done by setting a base condition of if n is 0 and if it isn't add n and call the method with n - 1. The second could be solved by setting the base condition of the exponent to be 1, which would return the base. If it wasn't, return base times the method with base and the exponent - 1."),
    new item("code", `public class Set1 {
    
        // Precondition: n is a nonnegative integer
        public int sum(int n) {
            if (n == 0) { // Base case
                return 0;
            } else { // Recursive case (Anything else)
                return n + sum(n - 1);
            }
        }
    
        // Precondition: exponent is a positive integer
        public int timeTable(int base, int exponent) {
            if (exponent == 1) { // Base case
                return base;
            } else { // Recursive case (Anything else)
                return base * timeTable(base, exponent - 1);
            }
        }
        ...
        public static void main(String[] args) {
            ...
            System.out.println("Testing with value 10: " + s.sum(10));
            System.out.println("Testing with value 100: " + s.sum(100));
            System.out.println("Testing with value 32: " + s.sum(32));
    
            System.out.println();
            System.out.println("Testing timeTable():");
            System.out.println("Testing with base of 12 and exponent of 3: " + s.timeTable(12, 3));
            System.out.println("Testing with base of 32 and exponent of 5: " + s.timeTable(32, 5));
            System.out.println("Testing with base of 3 and exponent of 12: " + s.timeTable(3, 12));
        }
    }`, "Set1.java"),
]